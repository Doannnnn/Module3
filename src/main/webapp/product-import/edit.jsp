...
<div class="container">
  <form action="/product-import?action=edit&id=${productImport.id}" method="post">
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="code" name="code" value="${productImport.code}" required>
    </div>
    <div class="mb-3">
      <label for="importDate" class="form-label">Ngày import</label>
      <input type="date" class="form-control" id="importDate" name="importDate" value="${productImport.importDate}" required>
    </div>
            <div class="mb-3">
                <label for="totalAmount" class="form-label">Tổng giá trị</label>
                <input type="number" class="form-control" id="totalAmount" name="totalAmount" required value="${productImport.totalAmount}" readonly>
            </div>
    <div class="row mb-3">
      <div class="col-4">
        Product
    ...
    <div id="product-import-detail">
      <c:forEach var="detail" items="${productImportDetails}" varStatus="status">
        <div class="row mb-3" id="product-import-${status.index}">
          <div class="col-4">
            <select class="form-control" name="productIds" id="product-${status.index}" onchange="onChangeSelect(this)" required>
              <option value="">---Please Choose---</option>
              <c:forEach var="product" items="${products}">
                <option value="${product.id}" <c:if test="${product.name.equals(detail.product.name) }">selected</c:if>>${product.name}</option>
              </c:forEach>
            </select>
          </div>
          <div class="col-3">
            <input type="number" class="form-control" name="quantities" required value="${detail.quantity}">
          </div>
          <div class="col-3">
            <input type="number" class="form-control" name="amounts" required value="${detail.amount}">
          </div>
          <div class="col-2 d-flex justify-content-end">
            <button type="button" class="btn btn-danger" onclick="deleteRow(${status.index})">Delete</button>
          </div>
        </div>
      </c:forEach>
    </div>
...
<script>
  ...
  let rowProductImport = ${productImportDetails.size()};
